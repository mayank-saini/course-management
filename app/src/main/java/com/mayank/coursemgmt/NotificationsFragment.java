...
import static com.mayank.coursemgmt.CurrentSem.getCurrSemesterShort;
...
public class NotificationsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return rootView = inflater.inflate(R.layout.fragment_notifications, null);
    }

    @Override
    public void onViewCreated(@NonNull final View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        nrecyclerview = rootView.findViewById(R.id.rvnotifications);
        nlist = new ArrayList<>();
        ...
        attendanceDB.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (itcount > 0) {
                    nlist.add(itcount + " student(s) from Information Technology have low attendance.");
                }
                if (cscount > 0) {
                    nlist.add(cscount + " student(s) from Computer Science have low attendance.");
                }
                if (mechcount > 0) {
                    nlist.add(mechcount + " student(s) from Mechanical have low attendance.");
                }
                if (civilcount > 0) {
                    nlist.add(civilcount + " student(s) from Civil have low attendance.");
                }
